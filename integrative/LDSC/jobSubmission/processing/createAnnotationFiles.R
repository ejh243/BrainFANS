...
for(chr in 1:22) {
	annotation_file <-
		read.table(gzfile(paste0(baseline_file_prefix, chr, ".annot.gz")),
							header = TRUE)
	...
	annotation_file <-
		annotation_file %>%
		select_if(function(x) all(is.integer(x) | x %in% c(0,1)))
	...
	snp_bed<-annotation_file[,c("CHR", "BP", "BP")]
	...
	any_regulatory_feature <- as.numeric(rowSums(annotation_file[,-c(1:3)]) > 0)

	output_annotation <-
  data.frame(snps$V1, snps$V4, snps$V2, snps$V3,
             mergeOverlaps, any_regulatory_feature, annotation_file$base)
