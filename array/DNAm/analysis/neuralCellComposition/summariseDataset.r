...
fig2<-list()
fig2[[1]]<-ggplot(pcDatAll, aes(x=PC1, y = PC2, color = Cell.type)) + geom_point(size = 4) + scale_color_manual(values = ctCols,
                           guide = guide_legend(override.aes = list(shape = 15, size = 10) )) + theme(text=element_text(size=21), legend.direction="horizontal") + labs(tag = letters[1]) + labs(color=NULL)
...
ggarrange(plotlist=fig2, nrow = 2, ncol = 3, common.legend = TRUE, legend.grob = get_legend(fig2[[1]], position = NULL))
