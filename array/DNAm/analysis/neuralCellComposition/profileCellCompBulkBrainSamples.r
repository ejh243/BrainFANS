...
if(length(unique(pheno$BrainRegion)) > 1){  
	...
	write.csv(lmOut, file = paste0(strsplit(bulkPath, "\\.")[[1]][1], "RegressionAgainstCETYGOANOVA.csv"))
	lmOut<-NULL
	...
	colnames(lmOut)<-c("Panel","AgeEst", "Age2P", "Age2Est", "Age2P", "SexEst", "SexP", c(outer(c("Est_", "P_"), levels(relevel(as.factor(pheno$BrainRegion), "BA9"))[-1], paste0)))
	write.csv(lmOut, file = paste0(strsplit(bulkPath, "\\.")[[1]][1], "RegressionAgainstCETYGOIDOL.csv"))
} else {
	lmOut<-NULL
	for(i in 1:length(predCCANOVA)){
		model<-lm(predCCANOVA[[i]][,"CETYGO"] ~ Age + Age2 + Sex + Sentrix_ID, data = pheno)
		lmOut<-rbind(lmOut, c(i, c(t(summary(model)$coefficients[c("Age", "Age2", "SexM"),c(1,4)]))))
	}
	colnames(lmOut)<-c("Panel","AgeEst", "Age2P", "Age2Est", "Age2P", "SexEst", "SexP")
	write.csv(lmOut, file = paste0(strsplit(bulkPath, "\\.")[[1]][1], "RegressionAgainstCETYGOANOVA.csv"))
	lmOut<-NULL
	for(i in 1:length(predCCIDOL)){
		if(!is.null(predCCIDOL[[i]])){
...
for(br in unique(pheno$BrainRegion)){
	...
	fig3b<-list()
	for(i in 1:length(predCCANOVA)){
		fig3a[[i]]<-ggplot(na.omit(subset(sumOut, BrainRegion == br & Method == "ANOVA")), aes(x=Model, y=CETYGO))  +
			geom_violin(position = pos, scale = 'width')  +
			stat_summary(fun = "mean", 
               geom = "point", 
               position = pos) + 
		...
		if(!is.null(predCCIDOL[[i]])){	
			fig3b[[i]]<-ggplot(na.omit(subset(sumOut, BrainRegion == br & Method == "IDOL")), aes(x=Model, y=CETYGO))  +
				geom_violin(position = pos, scale = 'width')  +
				stat_summary(fun = "mean", 
				   geom = "point", 
				   position = pos) + 
	...
	ggsave(filename = file.path(plotPath, paste0("ViolinPlotCETYGOAcrossModels",br,"SamplesANOVA.pdf")),  units = "in", width = 12, height = 8)
	ggarrange(plotlist=fig3a, nrow = 2, ncol = 4)
	ggsave(filename = file.path(plotPath, paste0("ViolinPlotCETYGOAcrossModels",br,"SamplesIDOL.pdf")),  units = "in", width = 12, height = 8)
...
predCCBest<-cbind(predCCANOVA[[1]][,c("DoubleNeg", "NeuNPos")], predCCIDOL[[5]][,c("IRF8Pos","TripleNeg")], predCCANOVA[[3]][,c("SATB2Neg", "SATB2Pos")], predCCANOVA[[6]][,c("NEUNNeg", "SOX6Pos", "SOX6Neg")], predCCIDOL[[4]][,"Sox10Pos"])
...
if(length(unique(pheno$BrainRegion)) > 1){  
	...
	colnames(lmOut)<-c("Panel","AgeEst", "Age2P", "Age2Est", "Age2P", "SexEst", "SexP", c(outer(c("Est_", "P_"), levels(relevel(as.factor(pheno$BrainRegion), "BA9"))[-1], paste0)))
	write.csv(lmOut, file = paste0(strsplit(bulkPath, "\\.")[[1]][1], "RegressionofAgeSexBrainRegionAgainstCellCompositionBest.csv"))
} else {
	for(j in 1:ncol(predCCBest)){
	...
	write.csv(lmOut, file = paste0(strsplit(bulkPath, "\\.")[[1]][1], "RegressionofAgeSexAgainstCellCompositionBest.csv"))
}
