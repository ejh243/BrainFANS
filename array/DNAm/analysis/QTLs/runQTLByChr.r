## Title: Run cis MatrixEQTL 
##
## Purpose of script: perform cis QTL analysis by chr across all samples and
## cell types
##
## Author: Eilis Hannon
##
...
output_file_name = file.path(sub("Input", "Output", dataDir), "LinearModel", paste0("cisQTLs_chr", chr, ".txt"))

if (!file.exists(file.path(sub("Input", "Output", dataDir), "LinearModel"))) {
 dir.create(file.path(sub("Input", "Output", dataDir), "LinearModel"), recursive = TRUE)
}
