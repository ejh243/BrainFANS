...
write.csv(cbind(QCmetrics[,c("Basename", "Samples.ID", "Indidivual.ID", "Cell.type")], QCSum),  paste0(dataDir, "/2_gds/QCmetrics/PassQCStatusAllSamples.csv"))
...
```{r, echo = FALSE}
nFMLab<-sum(QCmetrics$Indidivual.ID == "Meth Control")
nFMDat<-sum(QCmetrics$intens.ratio > 4)  
nFMCorrect<-sum(QCmetrics$Indidivual.ID == "Meth Control" & QCmetrics$intens.ratio > 4)
...
```{r,echo = FALSE, results = "asis"}
pander(t(as.data.frame(table(table(QCmetrics$Indidivual.ID)))), row.names = c("Number of Samples", "Number of Individuals"))
...
```{r,echo = FALSE, fig.width = 15, fig.height = 15}
  ...
  for(element in predictedduplicates){
	...
	if(length(errors) == 0){
	  ...
	  print("ERROR: Genetically identical samples had different individual IDs. See heatmap below for potential mismatches and output file WithinDNAmGeneticMismatches.csv for details")
	  errorIndexes<-NULL
	  ...
	  if("Indidivual.ID" %in% colnames(plotCols)){
  	  indCols<-plotCols$Indidivual.ID
	  } else {
	    indCols<-rainbow(length(unique(QCmetrics$Indidivual.ID)))[as.factor(QCmetrics$Indidivual.ID)]
	  }
	  
	heatmap.2(snpCor[errorIndexes,errorIndexes], trace = "none", key = FALSE, dendrogram = "column", ColSideColors = indCols[errorIndexes], labRow = QCmetrics$Basename[errorIndexes], labCol = QCmetrics$Indidivual.ID[errorIndexes], margin = c(10,10))
	if(length(errorIndexes) > 0){	
		write.csv(QCmetrics[errorIndexes,], paste0(dataDir, "/2_gds/QCmetrics/WithinDNAmGeneticMismatches.csv"), row.names = FALSE)
		}
	}

	## do heatmap across any individual who is not clustering as expected
	if(length(errorIndexes) > 0){
  samplePlot<-which(QCmetrics$Indidivual.ID %in% unique(QCmetrics$Indidivual.ID[errorIndexes]))
  pdf(paste0(dataDir, "/2_gds/QCmetrics/HeatmapGeneticInconsistencies.pdf"), height = 15, width = 10)
  heatmap.2(t(rsbetas[,samplePlot]), trace = "none", key = FALSE, dendrogram = "row", RowSideColors = c("red", "blue")[as.factor(QCSum[samplePlot, "snpCheck"])], labRow = QCmetrics$Indidivual.ID[samplePlot], margin = c(5,5), col = colorRampPalette(rev(brewer.pal(n = 7, name =
    "RdYlBu")))(100), 
      lhei=c(0.1,9), 
      lwid=c(2,9))	
  dev.off()
		...
		if(length(allGenoIDs) > 0){
			for(i in 1:length(bsnames)){
			  if(QCmetrics[match(bsnames[i], QCmetrics$Basename), "passQCS1"]){
				...
				methID<-sampleGroups$Indidivual.ID[match(bsnames[i], sampleGroups$barcode)]
...
xTab<-matrix(data = NA, ncol = ncol(QCSum), nrow = ncol(QCSum))
for(i in 1:ncol(QCSum)){
  for(j in 1:ncol(QCSum)){
    xTab[i,j]<-sum(rowSums(QCSum[,c(i,j)]) == 0, na.rm = TRUE)
  }
}
colnames(xTab)<-colnames(QCSum)
rownames(xTab)<-colnames(QCSum)
...
if(sexCheck & snpCheck){
corrplot(cor(QCmetrics[,c("M.median","U.median","intens.ratio",                                                                                            "bisulfCon",
 "PC1_cp","PC2_cp","PC3_cp","PC4_cp","PC5_cp",
 "PC1_betas","PC2_betas","PC3_betas","PC4_betas","PC5_betas","PC6_betas",
 "pFilter",
 "x.cp","y.cp",
 "genoCheck",
 "rmsd","sdd","sadd","srms",
...
passQC<-rownames(QCSum)[QCSum[,"passQCS2"]]
...
The age of samples can be predicted from the DNAm data using the Epigenetic Clock algorithm developed by Steve Horvath. These predicted values are compared to the samples' reported ages. As on a sample level this estimaion can be inaccurate it is used as a quality check of the overall data set and not as a reason to exclude individual samples. The overall correlation was `r if("Age" %in% colnames(QCmetrics)) {signif(cor(as.numeric(QCmetrics$Age), QCmetrics$DNAmAge, use = "pairwise.complete.obs"),3)}` and the root mean square error was `r if("Age" %in% colnames(QCmetrics)){signif(sqrt(mean((QCmetrics$Age-QCmetrics$DNAmAge)^2, na.rm = TRUE)),3)}` years. We also applied a custom clock algorithm developed secifically for cortical samples. The overall correlation was `r if("Age" %in% colnames(QCmetrics)){signif(cor(as.numeric(QCmetrics$Age), QCmetrics$CCDNAmAge, use = "pairwise.complete.obs"),3)}` and the root mean square error was `r if("Age" %in% colnames(QCmetrics)){signif(sqrt(mean((QCmetrics$Age-QCmetrics$CCDNAmAge)^2, na.rm = TRUE)),3)}` years.
