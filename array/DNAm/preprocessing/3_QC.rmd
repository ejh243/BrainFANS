...
plot(QCmetrics$x.cp, QCmetrics$y.cp, col = as.factor(QCmetrics$Sex), xlab = "X chromosome FC", ylab = "Y chromosome FC", pch = 16)
abline(v = 1)
abline(h = 1, pch = 16)
...
if("Age" %in% colnames(QCmetrics)){
  ...
  title(main = paste("RMSE = ", signif(sqrt(median(abs(QCmetrics$DNAmAge-QCmetrics$Age)^2, na.rm = TRUE)),3)), line = 2, adj = 1)
  abline(model, lty = 2)
  abline(a = 0, b = 1)
  ...
  title(main = paste("RMSE = ", signif(sqrt(median(abs(QCmetrics$CCDNAmAge-QCmetrics$Age)^2, na.rm = TRUE)),3)), line = 2, adj = 1)
  abline(model, lty = 2)
  abline(a = 0, b = 1)
}
```
