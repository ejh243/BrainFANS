...
for (element in predictedduplicates) {
  ...
  dupSampleIDs <-
    c(dupSampleIDs, paste(
      sort(QCmetrics$Indidivual.ID[index]),
      sep = "|",
      collapse = "|"
    ))
}
...
if (length(errors) == 0) {
  ...
  if ("Indidivual.ID" %in% colnames(plotCols)) {
    ...
    indCols <-
      rainbow(length(unique(QCmetrics$Indidivual.ID)))[as.factor(QCmetrics$Indidivual.ID)]
  }
  ...
  if (length(errorIndexes) > 0) {
    heatmap.2(
      ...
      labCol = QCmetrics$Indidivual.ID[errorIndexes],
      margin = c(10, 10)
    )
    write.csv(
...
if (length(errorIndexes) > 0) {
  samplePlot <-
    which(QCmetrics$Indidivual.ID %in% unique(QCmetrics$Indidivual.ID[errorIndexes]))
  pdf(
    paste0(
      dataDir,
  ...
  heatmap.2(
    ...
    labRow = QCmetrics$Indidivual.ID[samplePlot],
    margin = c(5, 5),
