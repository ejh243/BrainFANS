...
```{r, echo=FALSE, results='asis'}
if(exists("bimodP.y")){
  cat(paste0("Hartigans dip test for unimodality / multimodality, indicates we can ",
             if(bimodP.y < 0.05){"accept"} else {"reject"},
             " the alternative hypothesis of multiple modes (P = ",
             signif(bimodP.y,3), ")."))
}
```
...
```{r tabSexpredictions, echo = FALSE, eval = sexCheck}
if(bimodP.y < 0.05){
  DT::datatable(
...
if (ageExists) {
  # Check if Age is numeric, and convert if not
  if (!is.numeric(QCmetrics$Age)) {
    warning("Age column is not numeric. Attempting to convert.")
    QCmetrics$Age <- suppressWarnings(as.numeric(as.character(QCmetrics$Age)))
  }
