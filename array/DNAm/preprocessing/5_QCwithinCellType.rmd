...
author: "`r commandArgs(trailingOnly=T)[4]`"
output: 
  html_document:
    toc: true
    toc_float: true
...
knitr::opts_chunk$set(fig.align = "center", warning=FALSE, message=FALSE, echo = FALSE, cache = FALSE)
```
...
source(args[3]) ### change the content of this file to run QC with different thresholds
...
nCTStart<-table(QCmetrics$Cell.type)

QCmetrics<-QCmetrics[match(passQC, QCmetrics$Basename),]

nCTMid<-table(QCmetrics$Cell.type)
...
sample_anno<-QCmetrics[,c("Age","Sex", "Cell.type")[c("Age","Sex", "Cell.type") %in% colnames(QCmetrics)]]
...
plot(betas.scores[,1], betas.scores[,2],col = cellCols[as.factor(QCmetrics$Cell.type)], pch = 16, xlab = "PC1", ylab = "PC2", cex.axis = 1.5, cex.lab = 1.5)

plot(betas.scores[,3], betas.scores[,4],col = cellCols[as.factor(QCmetrics$Cell.type)], pch = 16, xlab = "PC3", ylab = "PC4", cex.axis = 1.5, cex.lab = 1.5)

legend("bottomright", pch = 16, col = cellCols, levels(as.factor(QCmetrics$Cell.type)))
...
for(i in 1:ncol(betas.scores)){
  ...
  boxplot(betas.scores[,i] ~ QCmetrics$Cell.type, col = cellCols, ylab = paste0("PC", i), xlab = "", cex.lab = 1.5, cex.axis = 1.5, xaxt = "n")
...
for(i in 1:2){
	boxplot(studentPCA[[1]][,i] ~ QCmetrics$Cell.type ,col = cellCols, pch = 16,ylab = "Studentized value", cex.axis = 1.5, cex.lab = 1.5, main = paste0("PC", i), xlab = "")
	abline(h=c(1,1.5,2,3), lty = 2)
	abline(h=c(1,1.5,2,3)*-1, lty = 2)
}
legend("topleft", pch = 16, col = cellCols, levels(as.factor(QCmetrics$Cell.type)))
```
...
#for(i in 1:ncol(pcaClassify$withinSDMean)){
 # plot(betas.scores[,1], betas.scores[,2], pch = #c(3,16)[as.factor(pcaClassify$withinSDMean[,i])], col = #cellCols[as.factor(QCmetrics$Cell.type)], xlab = "PC1", ylab = "PC2", cex = 1.2, #cex.lab = 1.5, cex.axis = 1.5)
#}
#legend("topleft", pch = 16, col = cellCols, #levels(as.factor(QCmetrics$Cell.type)))
...
for(i in 1:ncol(pcaClassify$withinSDMean)){
  ...
  plot(betas.scores[keep,1], betas.scores[keep,2], pch = 16, col = cellCols[as.factor(QCmetrics$Cell.type)][keep], xlab = "PC1", ylab = "PC2", cex = 1.2, cex.lab = 1.5, cex.axis = 1.5, main = paste0("Studentized threshold +/- ", c(1,1.5,2,3)[i]))
}
legend("bottomright", pch = 16, col = cellCols, levels(as.factor(QCmetrics$Cell.type)))
...
studCounts<-NULL
for(i in 1:ncol(pcaClassify$withinSDMean)){
  studCounts<-rbind(studCounts, table(QCmetrics$Cell.type[QCmetrics$maxStudentPCA > c(1,1.5,2,3)[i]]))
}
...
For each sample we can determine how similar it is to the average profile for the labelled cell type. We can combine the sample level scores into an individual level score, which is used to rank the FACs sorting. Below we will visualise the individual level FAC effciency scores. 
...
goodFACS<-c(uniqueIDs$Indidivual.ID[head(which(uniqueIDs$nFACS > 2), n = 3)], uniqueIDs$Indidivual.ID[head(which(uniqueIDs$nFACS > 3), n = 3)])
badFACS<-uniqueIDs$Indidivual.ID[tail(which(uniqueIDs$nFACS > 2), n = 3)]
...
for(each in goodFACS){
	...
	index<-which(QCmetrics$Indidivual.ID == each)
	plot(betas.scores[index,1], betas.scores[index,2],col = cellCols[as.factor(QCmetrics$Cell.type)][index], pch = c(18,16)[as.factor(QCmetrics$passCTCheck)][index], xlab = "PC1", ylab = "PC2", cex.axis = 1.5, cex.lab = 1.5, cex = 4, main = each, xlim = x_lim, ylim = y_lim)
...
badFACS<-uniqueIDs$Indidivual.ID[which(uniqueIDs$FACsEffiency >= 5)]
...
for(each in badFACS){
	...
	if(length(index > 1)){
    	...
    	mtext(side = 3, adj =1, signif(uniqueIDs$FACsEffiency[match(each, uniqueIDs$Indidivual.ID)],2))
	}
...
In total we identified, `r length(which(uniqueIDs$FACsEffiency >= 5))` individuals with sub optimal FACs sorting and all `r sum(!QCmetrics$passFACS & QCmetrics$Cell.type != "Total")` samples for these individuals were excluded.

```{r}

kbl(table(QCmetrics$Cell.type, QCmetrics$passFACS)) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), font_size = 10)
...
tab1<-table(QCmetrics$Cell.type,QCmetrics$predLabelledCellType)
...
upper.panel<-function(x, y){
  points(x,y, pch = 16, col = cellCols[as.factor(QCmetrics$Cell.type)])
  abline(a = 0, b = 1)
}
# Create the plots
pairs(mahDistPCA, 
...
for(each in colnames(mahDistPCA)){
	boxplot(mahDistPCA[,each] ~ QCmetrics$Cell.type, main = paste("Comparision with ", each), col = rainbow(5), ylab = "Mahalanobis distance", xlab = "Labelled cell type")
}
```
...
```{r}
tab2<-table(QCmetrics$Cell.type, QCmetrics$withinSDMean)
...
for(i in 1:ncol(lowerBound)){
  ...
  # just plot points labelled as that cell type
    sInd<-which(QCmetrics$Cell.type == colnames(lowerBound)[i])
...
if(length(cellTypes) < 10){
  pheatmap(rawbetas[order(sigma, decreasing = TRUE)[1:500],QCmetrics$withinSDMean], annotation_col = sample_anno[QCmetrics$withinSDMean,],  show_colnames = FALSE, show_rownames = FALSE, cutree_cols = length(cellTypes), main = "PCA Outliers")
} else {
    pheatmap(rawbetas[order(sigma, decreasing = TRUE)[1:500],QCmetrics$withinSDMean], annotation_col = sample_anno[QCmetrics$withinSDMean,],  show_colnames = FALSE, show_rownames = FALSE, main = "PCA Outliers")
}
...
passTab<- table(QCmetrics$passCTCheck, QCmetrics$Cell.type)
...
boxplot(QCmetrics$maxSD ~ QCmetrics$Cell.type, xlab = "Cell type", ylab = "nSD from mean",col = cellCols)

boxplot(QCmetrics$maxSD[QCmetrics$passCTCheck] ~ QCmetrics$Cell.type[QCmetrics$passCTCheck], xlab = "Cell type", ylab = "nSD from mean",col = cellCols)
...
#plot(betas.scores[passQC,1], betas.scores[passQC,2], xlab = "PC 1", ylab = "PC 2", col = cellCols[as.factor(QCmetrics$Cell.type)], pch = 16,cex.axis = 2, cex.lab = 2,cex = 1.2)
...
for(i in 1:ncol(betas.scores)){
  ...
  boxplot(betas.scores[,i] ~ QCmetrics$Cell.type, col = cellCols, ylab = paste0("PC", i), xlab = "", subset = rownames(betas.scores) %in% passQC)
...
countInd<-table(QCmetrics$Indidivual.ID[!QCmetrics$passCTCheck])
