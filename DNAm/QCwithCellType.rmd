...
## Summary of quality control thus far

After performing quality control (QC) across all samples (n = `r nStart`), `r nExclude`  samples were excluded after stages 1 and 2 of the QC. The first stage of the QC determines the DNAm data are of sufficient quality and stage determines whether the sample came from the correct individual. This third stage will look to establish the success of the FACs sorting and check whether samples cluster by their labelled cell type. 
...
percCon1<-sum(diag(tab1))/nrow(sampleSheet)*100
...
```{r}
tab2<-table(as.character(sampleSheet$Cell.type) == as.character(sampleSheet$closestCellTypePCA), sampleSheet$Cell.type == sampleSheet$pcaCellClassify & !is.na(sampleSheet$pcaCellClassify))
...
Let's compare the accuracy of the two methods

Here is a plot of the first two PCs to visualise how the two methods differ. The boxes in the panel on the far right, highlight the area on the graph within which each cell type is assigned. NB, in reality this algorithm is performed in higher dimensional space.
...
legend("topleft", pch = 16, col = palette()[1:length(cellTypes)], colnames(lowerBound))
plot(betas.scores[,1], betas.scores[,2], pch = c(4,1)[as.factor(as.character(sampleSheet$Cell.type) == as.character(sampleSheet$closestCellTypePCA))], xlab = "PC 1", ylab = "PC 2", col = as.factor(sampleSheet$Cell.type), main = "Method 1")
points(cellMedPCA[1,], cellMedPCA[2,], pch = 16, col = as.factor(colnames(cellMedPCA)))

plot(betas.scores[,1], betas.scores[,2], xlab = "PC 1", ylab = "PC 2", col = as.factor(sampleSheet$Cell.type), pch = c(4,1)[as.factor(sampleSheet$Cell.type == sampleSheet$pcaCellClassify)], main = "Method 2")
## add in NAs
points(betas.scores[is.na(sampleSheet$pcaCellClassify),1], betas.scores[is.na(sampleSheet$pcaCellClassify),2],col = as.factor(sampleSheet$Cell.type[is.na(sampleSheet$pcaCellClassify)]), pch = 3)
legend("topleft", pch = c(4,1,3), c("Discordant","Concordant",  "Not predicted"))
