...
sample_anno<-sampleSheet[,c("Cell.type", "Age","Sex", "Brain.Region", "Indidivual.ID")]
sample_anno$Age<-as.numeric(sample_anno$Age)
rownames(sample_anno)<-sampleSheet$Basename
sigma<-list()

for(each in unique(paste(studyInd, tissueInd)){
	eachStudy<-strsplit(each ," ")[1]
	eachBR<-strsplit(each ," ")[2]
	sigma[[each]]<-apply(rawbetas[,which(studyInd == eachStudy & tissueInd == eachBR)],1, sd)
	
	pheatmap(rawbetas[order(sigma[[each]], decreasing = TRUE)[1:1000],which(studyInd == each)], annotation_col = sample_anno[which(studyInd == each),],  show_colnames = FALSE, show_rownames = FALSE, cutree_cols = 4, main = paste(eachStudy, ":", eachBR))
}
...
```{r mahalanobisDist, }
mahDist<-rep(NA, nrow(sampleSheet))
pca<-prcomp(t(na.omit(rawbetas)))
varExp<-pca$sdev^2/sum(pca$sdev^2)*100
pcaUse<-which(varExp > 1)
cellInd<-sampleSheet$Cell.type
for(each in unique(paste(studyInd, tissueInd))){
	eachStudy<-unlist(strsplit(each ," "))[1]
	eachBR<-unlist(strsplit(each ," "))[2]
	sampleKeep<-which(studyInd == eachStudy & tissueInd == eachBR)
	cellCenter<-aggregate(pca$x[sampleKeep,pcaUse], by = list(cellInd), mean)
	for(eachCT in unique(cellInd[sampleKeep])){
		cellVar[[eachCT]]<-var(pca$x[sampleKeep[which(cellInd[sampleKeep] == eachCT)],pcaUse])
	}
	for(i in sampleKeep){
		rowN<-match(cellInd[i], cellCenter[,1])
		mahDist[i]<-mahalanobis(pca$x[i,pcaUse], as.numeric(cellCenter[rowN,-1]), cellVar[[cellInd[i]]])
	}
}
