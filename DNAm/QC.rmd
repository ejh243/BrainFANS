...
Data was loaded for `r nrow(QCmetrics)` samples from `r length(unique(QCmetrics$Project))` studies. 

```{r studyTable, results = "asis", echo = FALSE}

pander(as.data.frame(table(QCmetrics$Project)), col.names = c("Project", "Nsamples"))
...
for(item in techVar){
  tabTemp<-as.data.frame(table(QCmetrics[,item],QCmetrics$Project))
  if(nrow(tabTemp) < 11){
  	pander(t(tabTemp), caption = paste("Summary of samples categorised by", item), row.names = c("Variable", paste("Number of Samples", levels(QCmetrics$Project))))
  }
}
...
for(item in bioVar){
	pander(as.data.frame(table(QCmetrics[,item], QCmetrics$Project)), caption = paste("Summary of samples categorised by", item), col.names = c("Variable", paste("Number of Samples", levels(QCmetrics$Project))))
}
...
for(each in levels(QCmetrics$Project)){
  hist(QCmetrics$intens.ratio[which(QCmetrics$Project == each)], xlab = "Ratio of M:U intensities", breaks = 25, main = each, col = "gray")
...
for(each in levels(QCmetrics$Project)){
  hist(QCmetrics$bisulfCon[which(QCmetrics$Project == each)], xlab = "Bisulfite conversion (%)", breaks = 25, main = each, col = "gray")
  abline(v = 80, col = "red")
...
for(each in levels(QCmetrics$Project)){
   hist(snpCor[which(QCmetrics$Project == each),which(QCmetrics$Project == each)], breaks = seq(-1,1,0.05), xlab = "SNP correlation", main = each, col = "gray")
abline(v = 0.8, col = "red")
...
for(each in levels(QCmetrics$Project)){
    print(paste("Project:", each))
    pander(t(as.data.frame(table(table(predictedduplicates[which(QCmetrics$Project == each)])))), row.names = c("Number of Samples", "Number of Individuals"))
}
...
## separate by study
if(length(unique(QCmetrics$Project)) > 1){
  for(each in unique(QCmetrics$Project)){
    if("Indidivual.ID" %in% colnames(plotCols)){
      index<-which(QCmetrics$Project == each)
  		heatmap.2(snpCor[index,index], trace = "none", key = FALSE, dendrogram = "column", ColSideColors = plotCols$Indidivual.ID[index], RowSideColors = plotCols$Indidivual.ID[index], labCol = QCmetrics$Indidivual.ID[index], labRow = "", margins = c(6,6), main = each)
  	} else {
  		heatmap.2(snpCor[index,index], trace = "none", key = FALSE, dendrogram = "column", labCol = "", labRow = "", margins = c(1,1), main = each)
  	}
...
pander(table(QCmetrics$predSex.x, QCmetrics$predSex.y, QCmetrics$Project))
``` 

Second ,we will compare the sex predictions to that provided in the phenotype file

```{r tabSexReported, echo = FALSE}
pander(table(QCmetrics$predSex, QCmetrics$Sex, QCmetrics$Project))
``` 
...
pander(table(QCmetrics$genoCheck > 0.8, QCmetrics$Project), caption = "Number of samples where genotypes concordant across SNP and DNAm arrays")
