...
pander(as.data.frame(table(QCmetrics$Project)),table.attr = "style='width:50%'", col.names = c("Project", "Nsamples"))
...
if(nFail > 0){
	write.csv(QCmetrics[QCmetrics$genoCheck < 0.8,], file = "../../DNAm/QCmetrics/CompSNPdataGeneticErrors.csv")
}
```
...
```{r genoMatch, echo = FALSE}

which(!is.na(QCmetrics$genoMatch) & QCmetrics$genoCheck < 0.8)
