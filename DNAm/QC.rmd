...
```{r studyTable, results = "as.is", echo = FALSE}

pander(as.data.frame(table(QCmetrics$Project)),table.attr = "style='width:50%'", colnames = c("Project", "Nsamples"))
...
```{r techVarSpecification, results = "as.is", echo = FALSE}
...
```{r bioVarSpecification, echo = FALSE, results = "as.is", echo = FALSE}
...
Here we plot scatterplots of each PC against PC 1 to check for the presence of outliers.
...
pander(as.data.frame(table(table(predictedduplicates))), col.names = c("Number of Samples", "Number of Individuals"),table.attr = "style='width:50%'")
  ...
  for(each in unique(QCmetrics$Project)){
    if("Indidivual.ID" %in% colnames(plotCols)){
      index<-which(QCmetrics$Project == each)
  		heatmap.2(snpCor[index,index], trace = "none", key = TRUE, dendrogram = "column", ColSideColors = plotCols$Indidivual.ID[index], RowSideColors = plotCols$Indidivual.ID[index], labCol = QCmetrics$Indidivual.ID[index], labRow = "", margins = c(6,6))
  	} else {
  		heatmap.2(snpCor[index,index], trace = "none", key = FALSE, dendrogram = "column", labCol = "", labRow = "", margins = c(1,1))
  	}
...
`r sum(!is.na(QCmetrics$predSex))` samples had the same sex predicted on both the X and Y chromosome.


`r length(which(as.character(QCmetrics$predSex) == as.character(QCmetrics$Sex)))` samples were predicted the same sex as reported in the phenotype file and `r length(which(as.character(QCmetrics$predSex) != as.character(QCmetrics$Sex)))` samples were predicted the opposite sex to that provided in the phenotype file.  
...
#### Step 7: Compare with SNP data

To check for potential sample swaps we will compare the 59 SNPs on the EPIC array to genotype data we have. Genotype data was available for `r` samples. 

```{r genoCheck, results = "asis", echo = FALSE}

pander(table(QCmetrics$genoCheck > 0.95))
nFail<-length(which(QCmetrics$genoCheck == FALSE))
nRematch<-sum(!is.na(QCmetrics$genoMatch[QCmetrics$genoCheck == FALSE]))


if(nFail > 0){
	write.csv(QCmetrics[QCmetrics$genoCheck == FALSE,], file = "../../DNAm/QCmetrics/CompSNPdataGeneticErrors.csv")
}
```
...
```{r, betasPCAHist, fig.width = 15, fig.height = 4, echo = FALSE}
par(mar = c(4,4,4,1))
