...
for(item in techVar){
	knitr::kable(as.matrix(table(QCmetrics[,item])), caption = paste("Summary of samples categorised by", item))
}
...
kable(as.matrix(table(table(predictedduplicates))))
...
```{r tableSexMismatches}

kable(as.matrix(table(as.character(QCmetrics$predSex) != as.character(QCmetrics$Sex), QCmetrics$intensPASS)
