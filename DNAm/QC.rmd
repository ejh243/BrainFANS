...
```{r studyTable, results = "as.is"}

pander(as.data.frame(table(QCmetrics$Study)),table.attr = "style='width:50%'")
...
for(item in techVar){
	pander(as.data.frame(table(QCmetrics[,item])), format = "html", caption = paste("Summary of samples categorised by", item))
}
...
write.csv(QCmetrics[errorIndexes,], "GeneticErrors.csv", row.names = FALSE)
