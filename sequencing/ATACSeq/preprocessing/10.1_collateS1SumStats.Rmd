...
for (x in 1:3){
  ...
  if (min < refs[1, x]){
    labs[1, x]<-mean(c(min, refs[1, x]))
  } else { labs[1, x]<-refs[1, x]-sd(eMetrics[,colNum], na.rm = TRUE) } 
  if(max > refs[2, x]){
    labs[3, x]<-mean(c(max, refs[2, x]))
  } else { labs[3, x]<- refs[2, x]+sd(eMetrics[,colNum], na.rm = TRUE) }
