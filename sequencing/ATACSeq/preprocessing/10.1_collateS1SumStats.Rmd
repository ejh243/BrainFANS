...
for (x in 1:3){
  ...
  p[[x]]<-dplyr::select(eMetrics, all_of(colNum)) %>%
    ...
    geom_vline(xintercept = thres[,x],alpha=0.9, linetype="dashed", show.legend =FALSE)+
...
QCPASS<-cbind(mergeStats[,1] > 10*10^6,
              mergeStats$overall_alignment_rate > 80, #alignment rate 
              fripStats$BAMTotalReads > 20*10^6, #total filtered/aligned reads
