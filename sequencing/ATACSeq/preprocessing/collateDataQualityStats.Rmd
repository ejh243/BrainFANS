...
## load and calculate fraction reads in peaks
...
data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=propNucleosomesAll[,2], y=propNucleosomesAll[,1]))+
    ...
    geom_hline(yintercept = c(nfrThres))+
    geom_vline(xintercept = c(monoThres)) +
    geom_polygon(data = data.frame("x" = c(monoThres, nfrThres, 1,monoThres), "y" = c(nfrThres, nfrThres, 1, 1)), aes(x = x, y = y), fill = "grey", alpha = 0.2, size = 2)
...
Peak calling was performed in two ways using MACS, 1) using shifted tag align files, 2) using the paired end reads. ENCODE recommends that number of peaks in each sample be greater than 150,000, though values greater than 100,000 are acceptable. Data from `r sum(!is.na(fripStats$MACS2TagAlignPeaks))` samples is plotted below. The mean number of peaks is `r signif(mean(fripStats$MACS2TagAlignPeaks, na.rm = TRUE ),3)` (SD = `r signif(sd(fripStats$MACS2TagAlignPeaks, na.rm = TRUE ),3)`). The plots below contain data from `r sum(!is.na(fripStats$MACS2TagAlignPeaks))` samples. 
...
data.frame(fripStats$MACS2TagAlignPeaks, fripStats$MACS2PEPeaks)%>%
  ...
  ggplot(aes(y = value, x=variable))+
      ...
      stat_summary(fun="median", 
                 geom="point", color="white")+
      geom_hline(yintercept = c(150000, 100000), color = c('black', colorBlindGrey8[2]))+
...
x=1
for (i in 1:3){
    p[[x]]<-data.frame(fripStats$MACS2TagAlignPeak, pheno[,cols[i]]) %>%
      ggplot(aes(y = .[,1], x=as.factor(.[,2]), fill = as.factor(.[,2])))+
      geom_violin(scale='width')+
      geom_boxplot(width=0.05, fill='black')+
      labs(x = cols[i+3], y = "Peaks (Shift)")+
      stat_summary(fun="median", 
                 geom="point", color="white")+
      theme_bw()+
      theme(axis.text.y = element_text(angle = 90), 
   ...
   p[[x+1]]<-data.frame(fripStats$MACS2PEPeaks, pheno[,cols[i]]) %>%
      ggplot(aes(y = .[,1], x=as.factor(.[,2]), fill = as.factor(.[,2])))+
      geom_violin(scale='width')+
      geom_boxplot(width=0.05, fill='black')+
      labs(x = cols[i+3], y = "Peaks (PE)")+
      stat_summary(fun="median", 
                 geom="point", color="white")+
      theme_bw()+
      theme(axis.text.y = element_text(angle = 90), 
...
Peak calling will be affected by the background noise of a sample, and the success of the ATAC enrichment. Let's compare some of our periodicity statistics agains the number of peaks. 

```{r, fig.height = 16 , fig.width = 8}
p<-NULL
p[[1]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$MACS2TagAlignPeak, y=propNucleosomesAll[,1]))+
    geom_point()+
    theme_bw()+
...
p[[2]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$MACS2TagAlignPeak, y=propNucleosomesAll[,2]))+
    geom_point()+
    theme_bw()+
...
p[[3]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$MACS2TagAlignPeak, y=diptestStatsAll[,"D"]))+
    geom_point()+
    theme_bw()+
...
p[[4]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$MACS2TagAlignPeak, y=diptestStatsAll[,"p.value"]))+
    geom_point()+
    theme_bw()+
...
p[[5]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$FripMACS2TagAlign, y=propNucleosomesAll[,1]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Proportion nucleosome free")

p[[6]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$FripMACS2TagAlign, y=propNucleosomesAll[,2]))+
    geom_point()+
    theme_bw()+
...
p[[7]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$FripMACS2TagAlign, y=diptestStatsAll[,"D"]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Hartigans Test Statistic")

p[[8]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$FripMACS2TagAlign, y=diptestStatsAll[,"p.value"]))+
    geom_point()+
    theme_bw()+
...
data.frame(fripStats$FripMACS2TagAlign, fripStats$FripMACS2PE)%>%
  ...
  ggplot(aes(y = value, x=variable))+
      ...
      stat_summary(fun="median", 
                 geom="point", color="white")+
      geom_hline(yintercept = c(0.3, 0.2), color = c('black', colorBlindGrey8[2]))+
      scale_x_discrete(labels = c("MACS2: Shifted tagalign", "MACS2: PE"))+
      theme_bw()
...
QCPASS<-cbind(mergeStats$overall_alignment_rate > alignRate, #alignment rate 
              ...
              #propNucleosomesAll[,1] > nfrThres, #fractions nucleosome free 
              #propNucleosomesAll[,2] > monoThres, #fractions mononucleosome 
              #decreasingProps[,1],#periodicity
              #fripStats$MACS2TagAlignPeak > peakThres, # number of peaks
              #fripStats$FripMACS2TagAlign > fripThres #frip
              diptestStatsAll[,"p.value"] < dipPThres #multimodality
              ) 

colnames(QCPASS)<-c("AlignmentRate", "UniqueReads", "NRF", "PBC1", "PBC2", 
                    "SignifPeriodicity")
...
corMergeStats<-cbind(rowMeans(mergeStats[,c(1,4)]),rowMeans(mergeStats[,c(2,5)]), rowMeans(mergeStats[,c(3,6)]),
                     ...
                     diptestStatsAll[,2],
                     fripStats$MACS2TagAlignPeak,
                     fripStats$FripMACS2TagAlign) 
colnames(corMergeStats)<- c('total\nreads', 'dedup', 'poor\nqual', 'mt\nreads', 'alignt\nrate', 'distinct\nreads', 
                            colnames(mergeStats[,c((ncol(mergeStats)-2):ncol(mergeStats))]),  "PropNRF", "PropMono", "DipP", "NPeaks", "FRIP")
