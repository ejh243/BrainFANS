...
#SBATCH --job-name=ATACQCSummaryStage1
...
then
	...
	module load Pandoc
	Rscript -e "rmarkdown::render('ATACSeq/preprocessing/collateDataQualityStats.Rmd', output_file=paste0(commandArgs(trailingOnly=TRUE)[1], '/QCOutput/stage1SummaryStats.html'))" "$PEAKDIR" "${SCRIPTDIR}" "$PROJECT"
fi
...
then
  ...
  module purge
	...
	module load Pandoc
	Rscript -e "rmarkdown::render('ATACSeq/preprocessing/collateComparePeaks.Rmd', output_file=paste0(commandArgs(trailingOnly=TRUE)[1], '/QCOutput/comparePeaksStats.html'))" "$PEAKDIR" "${SCRIPTDIR}" "$PROJECT"
fi

shift #move command line arguments so that $1 is no longer project but step

# FILTER option merged into previous step
