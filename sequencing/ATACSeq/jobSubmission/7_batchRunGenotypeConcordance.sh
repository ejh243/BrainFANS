...
if [ $# = 1 ] || [[ $2 =~ 'GENCHECK' ]]
then
	## collate the Sex checks and Genotype check into a r markdown report
	cd ${SCRIPTDIR}
	echo ${SCRIPTDIR}
  
	module load R/3.6.3-foss-2020a
	module load Pandoc
	Rscript -e "rmarkdown::render('ATACSeq/preprocessing/collateSampleChecks.Rmd', output_file=paste0(commandArgs(trailingOnly=TRUE)[1], '/QCOutput/stage2SummaryStats.html'))" "$PEAKDIR" "${SCRIPTDIR}" "$PROJECT"
fi
