...
##   SCRIPTS_DIR, PEAK_DIR_SAMPLES, RSCRIPTS_DIR, CONDA_ENV, CONDA                                                    ||
...
echo "Total number of samples: "" ""${#SAMPLES[@]}"""

##Select sample using --array number
sample=${SAMPLES[${SLURM_ARRAY_TASK_ID}]}
echo "Sample selected is ${sample}"
...
## option PEAKS-N: peak calling is performed using MACS3 in paired-end at sample level in narrow peak mode 
if [ $# = 1 ] || [[ $2 == 'PEAKS' ]]
then
  
  mkdir -p ${PEAK_DIR_SAMPLES}
  
cat <<EOF

|| Running STEP 3.2 of ATAC-seq pipeline: PEAKS. Peaks will be called on sample using MACS3 on paired-end and narrowpeak mode. ||

Sample specified by array number in command line is: ${sample}
Output directory is ${PEAK_DIR_SAMPLES}
...
EOF
  ...
  sh "${SUB_SCRIPTS_DIR}/collateCalcFrip.sh" 
