...
filePaths <- list.files(file.path(alignedDir,"/QCOutput/"), pattern = "FragmentDistribution_Batch_")
...
p<- ggplot(fripStats, aes(x=fripStats[,3], y = fripStats[,5], colour = pheno$fraction))+
    geom_point()+
    theme_bw()+
...
colnames(QCPASS)<-c("AlignmentRate", "UniqueReads", "NRF", "PBC1", "PBC2","NFR", "MNC", "Peaks", "FRIP", "Multimodality")
...
write.csv(QCPASS, file = file.path(metaDir, "/passStage1Status.csv"))
