...
for (i in 1:3){ #select pheno columns
  for (j in 1:2){ #select mergeStats columns
    p[[x]]<-data.frame(mergeStats[,j], pheno[,cols[i]]) %>%
      ...
      theme_bw() +
      theme(axis.text.y = element_text(angle = 90),axis.text.x = element_text(angle = 45),
...
write.csv(QCPASS.df, file = file.path(qc1pass))

write.table(names(keep[keep==TRUE]), file = file.path(qc1passList), 
...
write.csv(qcStats, file = file.path(qc1stats),row.names = FALSE)
```

Samples will now be filtered based on thresholds defined in the config file. The table below, summarises the number of samples that pass each QC step.

```{r}
kbl(colSums(QCPASS, na.rm = TRUE))
```

The table below, summarises the number of samples that fail each QC step.
```{r, echo = FALSE}
QCPASS <- QCPASS[,c("AlignmentRate", "UniqueReads", "NRF", "PBC1", "PBC2","NFR", "MNC", "Periodicity","Peaks", "FRIP", "Multimodality", "QCPASS")]
...
pheatmap(xTab,
         ...
         cluster_rows = FALSE, 
         cluster_cols = FALSE,
         display_numbers = matrix(sprintf("%d", round(xTab)), nrow = nrow(xTab)),
         main = "Co-failing Samples Heatmap")
