...
output: 
  html_document:
    ...
    code_folding: hide
params:
  configFile: !r commandArgs(trailingOnly=TRUE)[2]
---
...
fripStats$FripMACS3TagAlign <- fripStats$ReadsinTAPeaks/fripStats$BAMTotalReads
fripStats$FripMACS3PE <- fripStats$ReadsinBAMPeaks/fripStats$BAMTotalReads
fripStats$FripMACS3HMMRATAC <- fripStats$ReadsinHMMRATACPeaks/fripStats$BAMTotalReads
...
Peak calling was performed in three ways using MACS, 1) using shifted tag align files, 2) using the paired end reads, 3) HMMRATAC. ENCODE recommends that number of peaks in each sample be greater than 150,000, though values greater than 100,000 are acceptable. Data from `r sum(!is.na(fripStats$BAMPeaks))` samples is plotted below. The mean number of peaks for MACS3 PE Peaks is `r signif(mean(fripStats$BAMPeaks, na.rm = TRUE ),3)` (SD = `r signif(sd(fripStats$BAMPeaks, na.rm = TRUE ),3)`), for MACS3 TA Peaks is `r signif(mean(fripStats$TAPeaks, na.rm = TRUE ),3)` (SD = `r signif(sd(fripStats$TAPeaks, na.rm = TRUE ),3)`) and for MACS3 HMMRATAC Peaks is `r signif(mean(fripStats$HMMRATACPeaks, na.rm = TRUE ),3)` (SD = `r signif(sd(fripStats$HMMRATACPeaks, na.rm = TRUE ),3)`). The plots below contain data from `r sum(!is.na(fripStats$BAMPeaks))` samples. 

```{r numpeaks, results=FALSE}
data.frame(fripStats$TAPeaks, fripStats$BAMPeaks, fripStats$HMMRATACPeaks)%>%
  melt() %>%
  ggplot(aes(y = value, x=variable))+
      ...
      scale_x_discrete(labels = c("MACS3: Shifted tagalign", "MACS3: PE", "MACS3: HMMRATAC"))+
	scale_y_continuous(labels = label_number(suffix = "", scale = 1e-5))+
      theme_bw()

```

```{r rviolinPlotPeaks,fig.height = 16 , fig.width = 8}
p<-NULL
x=1
for (i in 1:3){
    p[[x]]<-data.frame(fripStats$TAPeaks, pheno[,cols[i]]) %>%
      ggplot(aes(y = .[,1], x=as.factor(.[,2]), fill = as.factor(.[,2])))+
      geom_violin(scale='width')+
      geom_boxplot(width=0.05, fill='black')+
      labs(x = cols[i+3], y = "Number of peaks TA (100000s)")+
      stat_summary(fun="median", 
                 geom="point", color="white")+
      theme_bw()+
      theme(axis.text.y = element_text(angle = 90), 
   ...
   p[[x+1]]<-data.frame(fripStats$BAMPeaks, pheno[,cols[i]]) %>%
      ggplot(aes(y = .[,1], x=as.factor(.[,2]), fill = as.factor(.[,2])))+
      geom_violin(scale='width')+
      geom_boxplot(width=0.05, fill='black')+
      labs(x = cols[i+3], y = "Number of peaks PE (100000s)")+
      stat_summary(fun="median", 
                 geom="point", color="white")+
      theme_bw()+
      theme(axis.text.y = element_text(angle = 90), 
   ...
   p[[x+2]]<-data.frame(fripStats$HMMRATACPeaks, pheno[,cols[i]]) %>%
      ggplot(aes(y = .[,1], x=as.factor(.[,2]), fill = as.factor(.[,2])))+
      geom_violin(scale='width')+
      geom_boxplot(width=0.05, fill='black')+
      labs(x = cols[i+3], y = "Number of peaks HMMRATAC (100000s)")+
      stat_summary(fun="median", 
                 geom="point", color="white")+
      theme_bw()+
      theme(axis.text.y = element_text(angle = 90), 
...
#### 3.1.1 MACS3 TA peaks

Below, results from calling peaks using MACS3 TA mode are shown. 
```{r, fig.height = 16 , fig.width = 8}
p<-NULL
p[[1]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$TAPeaks, y=propNucleosomesAll[,1]))+
    geom_point()+
    theme_bw()+
...
p[[2]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$TAPeaks, y=propNucleosomesAll[,2]))+
    geom_point()+
    theme_bw()+
...
p[[3]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$TAPeaks, y=diptestStatsAll[,"D"]))+
    geom_point()+
    theme_bw()+
...
p[[4]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$TAPeaks, y=diptestStatsAll[,"p.value"]))+
    geom_point()+
    theme_bw()+
...
p[[5]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$FripMACS3TagAlign, y=propNucleosomesAll[,1]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Proportion nucleosome free")

p[[6]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$FripMACS3TagAlign, y=propNucleosomesAll[,2]))+
    geom_point()+
    theme_bw()+
...
p[[7]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$FripMACS3TagAlign, y=diptestStatsAll[,"D"]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Hartigans Test Statistic")

p[[8]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$FripMACS3TagAlign, y=diptestStatsAll[,"p.value"]))+
    geom_point()+
    theme_bw()+
...
#### 3.1.3 MACS3 HMMRATAC peaks

Below, results from calling peaks using MACS3 HMMRATAC mode are shown. 
```{r, fig.height = 16 , fig.width = 8}
p<-NULL
p[[1]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$HMMRATACPeaks, y=propNucleosomesAll[,1]))+
    geom_point()+
    theme_bw()+
    labs(x= "Number of Peaks (x10000)", y = "Proportion nucleosome free")+
	scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))
...
p[[2]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$HMMRATACPeaks, y=propNucleosomesAll[,2]))+
    geom_point()+
    theme_bw()+
    labs(x= "Number of Peaks (x10000)", y = "Proportion mono-nucleosome") +
	scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))
...
p[[3]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$HMMRATACPeaks, y=diptestStatsAll[,"D"]))+
    geom_point()+
    theme_bw()+
    labs(x= "Number of Peaks (x10000)", y = "Hartigans Test Statistic") +
	scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))
...
p[[4]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$HMMRATACPeaks, y=diptestStatsAll[,"p.value"]))+
    geom_point()+
    theme_bw()+
    labs(x= "Number of Peaks (x10000)", y = "Hartigans Test P") +
	scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))
...
p[[5]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$FripMACS3HMMRATAC, y=propNucleosomesAll[,1]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Proportion nucleosome free")

p[[6]]<-data.frame(propNucleosomesAll)%>%
  ggplot(aes(x=fripStats$FripMACS3HMMRATAC, y=propNucleosomesAll[,2]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Proportion mono-nucleosome")
...
p[[7]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$FripMACS3HMMRATAC, y=diptestStatsAll[,"D"]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Hartigans Test Statistic")

p[[8]]<-data.frame(diptestStatsAll)%>% 
    ggplot(aes(x=fripStats$FripMACS3HMMRATAC, y=diptestStatsAll[,"p.value"]))+
    geom_point()+
    theme_bw()+
    labs(x= "FRIP", y = "Hartigans Test P")
...
```{r readsinpeaks}
# plot 
data.frame(fripStats$FripMACS3TagAlign, fripStats$FripMACS3PE, fripStats$FripMACS3HMMRATAC)%>%
  melt() %>%
  ggplot(aes(y = value, x=variable))+
      geom_violin(fill = "grey80", scale='width')+
      geom_boxplot(width=0.05, fill='black')+
...
p=NULL
title<-c('MACS3: TA', 'MACS3: PE', 'MACS3: HMMRATAC')
for (x in c(2,3,4)){
  y<-x+8
  z<-x*2
  p[[x]]<-ggplot(fripStats, aes(x=fripStats[,z], y = fripStats[,y], colour = pheno$fraction))+
    geom_point()+
    theme_bw()+
    labs(title = title[x-1], x = "Number of peaks (100000s)", y = "Fraction reads in peaks")+
    scale_color_manual(values = colorBlindGrey8[1:length(unique(pheno$fraction))])+
    scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))+
    theme(legend.title = element_blank(), 
          legend.position = 'bottom')

  #p[[5]]<-get_legend(p[[x]]) #get the legend to add as a common legend
  #p[[x]]<-p[[x]]+theme(legend.position = 'none')
}
p<-p[2:5] # remove the null item
p <- c()
p[[1]]<-ggplot(fripStats, aes(x=fripStats[,4], y = fripStats[,10], colour = pheno$fraction))+
    geom_point()+
    theme_bw()+
    labs(title = title[1], x = "Number of peaks (100000s)", y = "Fraction reads in peaks")+
    scale_color_manual(values = colorBlindGrey8[1:length(unique(pheno$fraction))])+
    scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))+
    theme(legend.title = element_blank(), 
          legend.position = 'bottom')+theme(legend.position = 'none')
p[[2]]<-ggplot(fripStats, aes(x=fripStats[,6], y = fripStats[,11], colour = pheno$fraction))+
    geom_point()+
    theme_bw()+
    labs(title = title[2], x = "Number of peaks (100000s)", y = "Fraction reads in peaks")+
    scale_color_manual(values = colorBlindGrey8[1:length(unique(pheno$fraction))])+
    scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))+
    theme(legend.title = element_blank(), 
          legend.position = 'bottom')+theme(legend.position = 'none')
p[[3]]<-ggplot(fripStats, aes(x=fripStats[,8], y = fripStats[,12], colour = pheno$fraction))+
    geom_point()+
    theme_bw()+
    labs(title = title[3], x = "Number of peaks (100000s)", y = "Fraction reads in peaks")+
    scale_color_manual(values = colorBlindGrey8[1:length(unique(pheno$fraction))])+
    scale_x_continuous(labels = label_number(suffix = "", scale = 1e-5))+
    theme(legend.title = element_blank(), 
          legend.position = 'bottom')
...
grid.arrange(grobs = p, ncol=3, nrow = 2,layout_matrix = rbind(c(1,2,3), c(4,4,4)),widths = c(2.7, 2.7,2.7), heights = c(2.5, 0.5))
...
QCPASS<-cbind(mergeStats$overall_alignment_rate > alignThres, #alignment rate 
              ...
              fripStats$TAPeaks > peakThres, # number of peaks
              fripStats$FripMACS3TagAlign > fripThres, #frip
              fripStats$BAMPeaks > peakThres, # number of peaks
              fripStats$FripMACS3PE > fripThres, #frip
              fripStats$HMMRATACPeaks > peakThres, # number of peaks
              fripStats$FripMACS3HMMRATAC > fripThres, #frip
              diptestStatsAll[,"p.value"] < dipPThres #multimodality
              ) 

colnames(QCPASS)<-c("AlignmentRate", "UniqueReads", "NRF", "PBC1", "PBC2","NPeaks_TA", "FRIP_TA","NPeaks_PE", "FRIP_PE","NPeaks_H", "FRIP_H","SignifPeriodicity")
rownames(QCPASS)<-processSum$sampleID

write.csv(cbind(pheno, QCPASS), file = file.path(metaDir, "/passStage1Status.txt"))
...
corMergeStats<-cbind(rowMeans(mergeStats[,c(1,4)]),rowMeans(mergeStats[,c(2,5)]), rowMeans(mergeStats[,c(3,6)]),
                     ...
                     mergeStats[,c((ncol(mergeStats)-2):ncol(mergeStats))],
                     propNucleosomesAll[,c(1,2)],
                     diptestStatsAll[,2],
                     fripStats$TAPeaks,
                     fripStats$FripMACS3TagAlign,
                     fripStats$BAMPeaks,
                     fripStats$FripMACS3PE,
                     fripStats$HMMRATACPeaks,
                     fripStats$FripMACS3HMMRATAC) 
colnames(corMergeStats)<- c('total\nreads', 'dedup', 'poor\nqual', 'mt\nreads', 'alignt\nrate', 'distinct\nreads', 
                            colnames(mergeStats[,c((ncol(mergeStats)-2):ncol(mergeStats))]),  "PropNRF", "PropMono", "DipP", "NPeaks_TA", "FRIP_TA","NPeaks_PE", "FRIP_PE","NPeaks_H", "FRIP_H")
