...
s1Pass <- read.csv(file.path(qc1pass),header = TRUE)
...
samplesPassed <- s1Pass[s1Pass$QCPASS==TRUE,]
nCTMid<-table(samplesPassed$fraction)
nSBMid<-table(samplesPassed$sequencingBatch)
nBBMid<-table(samplesPassed$cohort)
...
write.csv(passStage2, file = file.path(qc2pass))

# generate a txt file of passed samples
write.table(passStage2[passStage2$QCPASS==TRUE,]$sampleID, file = file.path(qc2passList), 
...
write.csv(QCPASSALL, file = file.path(qc12stats), row.names = FALSE)
```
