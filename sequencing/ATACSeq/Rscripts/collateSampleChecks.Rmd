...
QCPASS <- as.data.frame(QCPASS)
QCPASS$QCPASS[is.na(QCPASS$genoCheck)& QCPASS$sexCheck == TRUE]<- TRUE
QCPASS$QCPASS[is.na(QCPASS$genoCheck)& QCPASS$sexCheck == FALSE]<- FALSE
QCPASS$QCPASS[is.na(QCPASS$sexCheck)& QCPASS$genoCheck == FALSE]<- FALSE
QCPASS$QCPASS[is.na(QCPASS$sexCheck)& is.na(QCPASS$genoCheck)]<- FALSE
QCPASSALL<-cbind(passStage2, "QCS1" = s1Pass$QCPASS, "QCS2" = QCPASS$QCPASS)
#QCPASSALL$QCS2[is.na(QCPASSALL$QCS2)& QCPASSALL$sexCheck == TRUE]<- TRUE
...
write.csv(QCPASSALL, file = file.path(metaDir, "/passAllStatus.csv"), row.names = FALSE)
```
