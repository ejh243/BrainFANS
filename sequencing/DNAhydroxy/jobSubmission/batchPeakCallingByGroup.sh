...
#SBATCH --output=DNAhydroxy/logFiles/%u/HydroxyPeakCalling-%A_%a.o
#SBATCH --error=DNAhydroxy/logFiles/%u/HydroxyPeakCalling-%A_%a.e
#SBATCH --job-name=HydroxyPeakCalling
...
Rscript ../general/processing/makeGroupAnalysisFile.r "DNAhydroxy/${PROJECT}" "${tissue}"
...
FILES=( "${QC1SAMPLES[@]/%/*.deduplicated.bam}" )
...
then
	...
	sh ./DNAhydroxy/preprocessing/groupPeaks.sh ${FILES[@]} 
fi
...
then
	...
	sh ./DNAhydroxy/preprocessing/calcFripGroup.sh ${QC1SAMPLES[@]}
fi
...
cd ${SCRIPTDIR}/DNAhydroxy/logFiles/${USER}
