...
bedtools map -a ${REFGENOME}/genome.window.fa.bg -b ${fraction[${SLURM_ARRAY_TASK_ID}]}.bg -c 4 -o mean | grep -P "\d$" > $LOADDIR/${fraction[${SLURM_ARRAY_TASK_ID}]}'_wgbs.window.bg'