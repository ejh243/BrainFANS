...
sampleSheet<-read.csv("/gpfs/mrc0/projects/Research_Project-MRC190311/CUTRUN/QCmetrics/SampleSheetForCUTRUN.csv", stringsAsFactors = FALSE, row.names = 1)

load("/gpfs/mrc0/projects/Research_Project-MRC190311/CUTRUN/QCmetrics/ChIPQCMetrics.rdata")
```


This report primarily makes use of [ChIPQC R package](http://bioconductor.org/packages/release/bioc/html/ChIPQC.html) downloaded from Bioconductor. Raw fastq files have been have been aligned with Bowtie2 to the human genome (hg38) and peaks called with MACS2 for each individual sample. 
...
boxplot(ssd(dat) ~ sampleSheet$Factor, cex.axis = 2, cex.lab = 2, ylab = "SSD", xlab = "Mark", main = "")
...
plotRap(dat) + theme(text = element_text(size=20), axis.title = element_text(size = 10, angle = 25))# + facet_wrap(~ sampleSheet$Factor)
...
barplot(frip(dat)*100, ylim = y_lim, ylab = "% Reads in Peaks", names.arg = sampleSheet$SampleID, 
        cex.names = 2, cex.axis = 2, cex.lab = 2, col = as.factor(sampleSheet$Factor))
