...
for FINAL_BAM_FILE in ${BAMFILES[@]};
do

	OFPREFIX=${FINAL_BAM_FILE%_trimmed_depDup_q30.bam}
	...
	Rscript ${PHANTOMPEAK}run_spp.R -c=${SUBSAMPLED_TA_FILE} -p=${NTHREADS} -filtchr=chrM -savp=${CC_PLOT_FILE} -out=${CC_SCORES_FILE}
