...
setwd(dataDir)
sampleSheet<-read.csv("SampleSheetForATACSeqQC.csv", stringsAsFactors = FALSE, row.names = 1)

## check if data already loaded and saved
if(file.exists("ATACSeqQCObject_1.rdata")){
	load("ATACSeqQCObject_1.rdata")
} else{
	print("QC data  1 not found")
}

if(file.exists("ATACSeqQCObject_2.rdata")){
...
fastqcStats<-read.table("multiqc_data/multiqc_fastqc.txt", sep = "\t", header = TRUE)
bowtieStats<-read.table("multiqc_data/multiqc_bowtie2.txt", sep = "\t", header = TRUE)
bamStats<-lapply(sub("_depDup_q30.bam", "_statsperchr.txt", sampleSheet$bamReads), read.table)
endcodeQC<-matrix(data = unlist(lapply(gsub("_depDup_q30.bam", ".nodup.pbc.qc", sampleSheet$bamReads), read.table)), ncol = 7, byrow = TRUE)

```

This report makes use of two R packages to summarise the performace on ATAC-seq, [ChIPQC R package](http://bioconductor.org/packages/release/bioc/html/ChIPQC.html) downloaded from Bioconductor, but it also uses the output from the [BBMap](https://sourceforge.net/projects/bbmap/) bbcountunique.sh tool. Raw fastq files have been have been aligned with Bowtie2 to the human genome (hg38) and peaks called with MACS2 for each individual sample. As this is pilot data the pipeline is still under development. The content of this report is based on a similar report generated for ChIP-Seq data.

## 1. Data summary


### 1.1 Sequencing data summary

Below is a table summarising the raw sequencing data we received. 

```{r, results = "asis", echo = FALSE, message = FALSE}


## get stats for r1
r1_dat<-fastqcStats[match(paste0(sampleSheet$SampleID, "_r1"), fastqcStats$Sample), c("Total.Sequences", "avg_sequence_length", "Sequence.length", "X.GC", "total_deduplicated_percentage")]

colnames(r1_dat)<-paste("r1", c("nReads", "Mean_length", "Range_length", "GC_percent", "total_deduplicated_percentage"), sep = "_")

## if paired end get stats for r2
r2_dat<-matrix(data = "NA", nrow = nrow(r1_dat), ncol = ncol(r1_dat))
if(sum(sampleSheet$ReadType == "Paired") > 0){
	r2_dat[which(sampleSheet$ReadType == "Paired"),]<-as.matrix(fastqcStats[match(paste0(sampleSheet$SampleID[which(sampleSheet$ReadType == "Paired")], "_r2"), fastqcStats$Sample),c("Total.Sequences", "avg_sequence_length", "Sequence.length", "X.GC", "total_deduplicated_percentage")])
}
colnames(r2_dat)<-paste("r2", c("nReads", "Mean_length", "Range_length", "GC_percent", "total_deduplicated_percentage"), sep = "_")



kable(cbind(r1_dat, r2_dat), digits = 2) %>%
	add_header_above(header = setNames(c(1, ncol(r1_dat),ncol(r2_dat)),c("", "Read 1","Read 2")))
...
```{r, plotUniquenessCurve, echo = FALSE, fig.height = 6, fig.width = 6, fig.caption = "Figure 1: Line graph of number of unique reads (y-axis) as number of reads (x-axis) increases.", message = FALSE,results='hide'}
...
```{r, fitMonodCurve, echo = FALSE, fig.width = 6, fig.height = 6, fig.caption = "Plot of number of unique reads against total number of reads", message = FALSE,results='hide', include = FALSE}
...
```{r, summarizeMonodFit, echo = FALSE, fig.width = 6, fig.height = 6, fig.caption = "Figure 2: Scatterplot of library complexity against sequencing efficiency",results='hide'}
...
legend("topright", sampleSheet$SampleID, pch = 16, col = lineCol, lwd = 2, cex = 1.5)
...
library(TxDb.Hsapiens.UCSC.hg38.knownGene) ## need to load here rather than beginning as conflicts with dplyr
...
#plot heatmap
featureAlignedHeatmap(sigs.log2[[1]], reCenterPeaks(TSS, width=ups+dws),
						  zeroAt=.5, n.tile=NTILE)
```

Second, the line graph below, plots the standardardized coverage of reads within 1kp of a TSS. The black solid line represents the nucleosome free fragments, which should peak at 0bp indictaing an enrichment of fragments at the TSS. The red dashed line represents the mononucleosome fragments, which should have a deression at 0, and peaks of signal either side. 
...
```{r, plotGenomicHeatmap, echo = FALSE, fig.width = 6, fig.height = 6, fig.cap="Heatmap genomic feature enrichment",results='hide'}
...
```{r, echo = FALSE, fig.width = 6, fig.height = 6, fig.cap="Plot of the average signal profile across peaks",results='hide'}
...
```{r, echo = FALSE, fig.width = 6, fig.height = 6, fig.cap="Barplot of the proportion of reads in called peaks"}
...
```{r, plotFrip, echo = FALSE, fig.width = 5, fig.height = 5, fig.cap="Barplot of the % fragments in peaks per sample"}
