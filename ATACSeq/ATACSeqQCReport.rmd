...
setwd(alignedDir)
sampleSheet<-read.csv(paste(dataDir, "SampleSheetForATACSeqQC.csv", sep = "/"), stringsAsFactors = FALSE, row.names = 1)
...
setwd(alignedDir)
fastqcStats<-read.table(paste(dataDir, "multiqc_data/multiqc_fastqc.txt", sep = "/"), sep = "\t", header = TRUE)
...
endcodeQC<-matrix(data = unlist(lapply(paste0("ENCODEMetrics", gsub(alignedDir, "", gsub("_depDup_q30.bam", ".nodup.pbc.qc", sampleSheet$bamReads))), read.table)), ncol = 7, byrow = TRUE)
...
```{r}
stats.tmp<-cbind(fastqcStats[match(paste0(sampleSheet$SampleID, "_r1"), fastqcStats$Sample), "Total.Sequences"], fastqcStats[match(paste0(sampleSheet$SampleID, "_r2"), fastqcStats$Sample), "Total.Sequences"], reads(dat),unlist(lapply(peaks(dat), length)))
...
## get stats for r1
r1_index<-match(sampleSheet$SampleID, unlist(lapply(strsplit(as.character(fastqcStats$Sample), "_"), head,n = 1)))
...
if(sum(sampleSheet$ReadType == "Paired") > 0){
	r2_dat<-fastqcStats[-r1_index,c("Total.Sequences", "avg_sequence_length", "Sequence.length", "X.GC", "total_deduplicated_percentage")]
}
